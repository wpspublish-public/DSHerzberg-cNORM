...
In this script, we pipe `norm_data` into `mutate()` to create a new column `weights`. Within `mutate()`, we execute `computeWeights()`, populating the new column with the weights for each person in `norm_data`.

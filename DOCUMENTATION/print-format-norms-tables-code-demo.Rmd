...
Because the data object is grouped by `ss`, we can call `dplyr::summarize()` to further collapse the table so there is a single row for each possible value of `ss`. `summarize()` operates within groups, and is used to aggregate the values of a variable, returning a new raw column containing a single summary value for each group.[^2]
...
```{r print-format-norms, echo = 41:42, eval = F}
```

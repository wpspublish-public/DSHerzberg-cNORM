...
With the data in this format, we are ready to define what constitutes "reversal" and identify those rows where it occurs. Recall that the fundamental cell-to-cell comparison for reversal was originally defined as between adjacent cells within the rows of the wide-format table. In the long-format object, those rows are now stacked on top of one another in columns, meaning that the *last* cell of the row for `raw = 1` is now adjacent to the *first* cell of the row for `raw = 2`, as can be seen in the subset of rows shown above. It is important that we exclude these "first-last" pairs from the reversal detection process, because the concept of a score reversal is only meaningful for adjacent age groups within a single raw score.
...
To refine the output, we call `filter(reversal == 1)`, which drops all rows except those were a reversal occurs. This allows the researcher to quickly identify reversed cell pairs in the larger basic format lookup table, rather than having to search the entire table for a possible "needle-in-a-haystack" type phenomenon. We use `select()` to drop the `reversal` marker column, which is now superfluous, and `write_csv()` the output report.

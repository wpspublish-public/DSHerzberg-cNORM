...
-   `group`: designates the age group column in the input data file
    (here `input$group`).
-   `k`: a power constant that sets the general limit on the expansion
    of the Taylor polynomial series[^2], which controls the precision of
    estimation of the normative model. The default value is 4, and
    values of 3 and 5 can be used in searching for an acceptable model.
